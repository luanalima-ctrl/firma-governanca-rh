...
def _narrativa_comportamental_offline(dominio_principal: str, forcas: str, fraquezas: str) -> str:
    return (
        f"Seu perfil demonstra uma forte inclinação para o Domínio de {dominio_principal}, com destaque especial "
        f"para suas principais forças: {forcas}. Em seu dia a dia profissional, essas competências se traduzem em "
        f"energia natural para conduzir suas frentes de trabalho. Como oportunidade de crescimento, focar nas "
        f"competências de {fraquezas} ajudará a equilibrar ainda mais sua performance na Firma."
    )


def gerar_narrativa_comportamental(dominio_principal: str, top_talentos: list, pontos_desenvolvimento: list) -> str:
    """Chama a API do Claude para gerar um perfil textual personalizado
    focado nos pontos fortes do colaborador, com fallback offline caso a
    chave não esteja configurada ou a chamada falhe."""
    forcas = ", ".join(t["talento"] for t in top_talentos)
    fraquezas = ", ".join(pontos_desenvolvimento)

    api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not api_key:
        return _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas)

    prompt = (
        "Você é um psicólogo organizacional sênior especialista em mapeamento de pontos fortes. "
        "Um colaborador da Firma Produções realizou o inventário comportamental interno e obteve o seguinte resultado técnico:\n"
        ...
        "Redija uma devolutiva de perfil profissional para este colaborador. O texto deve ser escrito em Segunda Pessoa ('Você'), "
        "ter um tom de alto padrão, encorajador, corporativo e focado em como ele pode usar essas 5 forças para crescer na empresa "
        "(seja atuando na fita técnica de eventos ou no backoffice). Explique o significado do domínio principal de forma prática. "
        "O texto deve ser direto, sem saudações ou preâmbulos, contendo de 2 a 3 parágrafos fluidos."
    )
    try:
        resp = requests.post(
            ANTHROPIC_API_URL,
        ...
        return texto or _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas)
    except Exception:
        return _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas)
...
def renderizar_resultado_comportamental(perfil: dict, titulo: str = "Perfil Comportamental"):
    ...
    st.info(f"**Domínio Comportamental Predominante:** {perfil['dominio_principal']}")

    col_top, col_dev = st.columns(2)
    with col_top:
        st.markdown("**Top 5 Talentos mais Evidentes**")
    ...
    if perfil.get("narrativa_ia"):
        st.markdown("**Análise de Perfil de IA**")
        st.info(perfil["narrativa_ia"])
