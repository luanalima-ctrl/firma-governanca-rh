...
NARRATIVA_CHAVES_TALENTO = ("talento", "definicao_curta", "o_que_representa", "como_aparece_comportamento", "como_aparece_trabalho", "quando_bem_utilizado", "ponto_atencao")
NARRATIVA_CHAVES_GAP = ("talento", "o_que_representa", "o_que_pode_acrescentar", "como_menor_evidencia_aparece", "onde_pode_ser_importante", "como_comecar_desenvolver")
NARRATIVA_CHAVES_TOPO = ("frase_sintese", "talentos_predominantes", "combinacao_talentos", "visao_geral_perfil", "talentos_desenvolver", "combinacao_gaps", "direcionamento", "sintese_final")
NARRATIVA_CHAVES_DIRECIONAMENTO = ("continue_potencializando", "fique_atento", "desenvolva", "experimente_na_pratica")
...
def _validar_narrativa_estruturada(parsed: dict) -> bool:
    ...
    quebraria a renderização em cards na tela."""
    try:
        ...
        for item in parsed["talentos_desenvolver"]:
            if not all(k in item for k in NARRATIVA_CHAVES_GAP):
                return False
        if not all(k in parsed["direcionamento"] for k in NARRATIVA_CHAVES_DIRECIONAMENTO):
            return False
        return True
...
def _narrativa_comportamental_offline(top_talentos: list, pontos_desenvolvimento: list, dominio_principal: str) -> dict:
    """Fallback offline com a mesma estrutura rica (cards por talento +
    combinações + direcionamento) exigida do modo online — usa as
    definições oficiais de DEFINICOES_TALENTOS, então nunca inventa
    característica sem relação com o talento, mesmo sem IA disponível."""
    ...
    for item in top_talentos:
        ...
        talentos_predominantes.append({
            ...
            "o_que_representa": definicao,
            "como_aparece_comportamento": f"Esse talento tende a aparecer nas situações em que você precisa decidir como agir no dia a dia, influenciando sua forma natural de responder a elas.",
            "como_aparece_trabalho": "Na rotina da Firma, essa força pode aparecer tanto no planejamento quanto na execução das suas atividades, adaptando-se ao contexto da sua função.",
            "quando_bem_utilizado": "Quando bem direcionado, esse talento gera resultado com naturalidade, sem exigir esforço consciente para sustentar o comportamento.",
            "ponto_atencao": "Como toda força, pode se tornar um ponto de atenção quando usada em excesso ou sob forte pressão, sendo percebida de forma diferente da intenção original.",
        })
    ...
    for nome in pontos_desenvolvimento:
        ...
        talentos_desenvolver.append({
            "talento": nome,
            "o_que_representa": definicao,
            "o_que_pode_acrescentar": "Desenvolver essa competência pode ampliar seu repertório de respostas diante de situações diferentes das que você já domina naturalmente.",
            "como_menor_evidencia_aparece": "Pode acontecer de essa não ser sua primeira resposta natural diante de uma situação — o que não significa incapacidade, apenas menor familiaridade.",
            "onde_pode_ser_importante": "Em determinados momentos da rotina profissional, essa competência pode ser exigida mesmo não sendo uma força natural do seu perfil.",
            "como_comecar_desenvolver": "Um bom começo é observar conscientemente situações em que essa competência é necessária e praticar deliberadamente uma resposta diferente da sua tendência natural.",
        })
    ...
    return {
        ...
        "visao_geral_perfil": (
            f"De forma geral, seu perfil de {dominio_principal} tende a definir seu ritmo natural de trabalho, a "
            "maneira como você se relaciona com a equipe e como toma decisões diante de desafios. Esses talentos "
            "tendem a gerar mais valor exatamente nas situações em que podem ser aplicados com autonomia."
        ),
        ...
        "direcionamento": {
            "continue_potencializando": [f"Continue aplicando {t['talento']} nas situações em que ele já gera bons resultados." for t in top_talentos[:3]],
            "fique_atento": [f"Observe se o excesso de {t['talento']} não está gerando ruído com a equipe sob pressão." for t in top_talentos[:2]],
            "desenvolva": [f"Pratique conscientemente situações que exijam {g}." for g in pontos_desenvolvimento[:3]],
            "experimente_na_pratica": ["Escolha uma situação da próxima semana para observar conscientemente como seus talentos aparecem na prática."],
        },
        "sintese_final": (
            f"Profissionalmente, você tende a ser alguém que combina {forcas_nomes} para conduzir o seu trabalho com "
            "consistência, contribuindo para a equipe através dessas forças naturais. Desenvolver gradualmente "
            f"{gaps_nomes} amplia ainda mais essa atuação, sem abrir mão do que já funciona bem. Seu perfil é um "
            "ponto de partida para se conhecer melhor — não um limite para o que você pode se tornar."
        ),
    }
...
def gerar_narrativa_comportamental(dominio_principal: str, top_talentos: list, pontos_desenvolvimento: list, cargo_colab: str = "", area_colab: str = "") -> dict:
    """Chama a API do Claude para gerar a devolutiva comportamental
    estruturada (JSON com um bloco detalhado por talento, a combinação
    entre eles, e o direcionamento prático) — não mais um texto corrido
    único. Sempre devolve um dicionário com a mesma estrutura, caindo no
    fallback offline se a chave não estiver configurada, se a resposta da
    IA vier incompleta ou se a chamada falhar."""
    ...
    prompt_devolutiva = (
        "Você é um Analista Comportamental Sênior. Sua missão é produzir uma devolutiva de autoconhecimento "
        "profissional — não um laudo clínico, não um diagnóstico de personalidade — a partir dos talentos oficiais "
        "identificados no inventário comportamental do colaborador.\n\n"
        f"[DOMÍNIO PREDOMINANTE] {dominio_principal}\n"
        f"[CARGO / ÁREA DO COLABORADOR] {contexto_profissional}\n\n"
        "[OS 5 TALENTOS PREDOMINANTES E SUAS DEFINIÇÕES OFICIAIS — use exclusivamente este significado, não invente características fora dele]\n"
        ...
        "[REGRAS OBRIGATÓRIAS]\n"
        "- Use apenas os talentos e definições acima. Não crie talentos novos, não altere os nomes.\n"
        "- Não trate o resultado como diagnóstico psicológico e não afirme que o perfil determina a personalidade da pessoa.\n"
        "- Não use os termos 'fraqueza', 'defeito', 'deficiência' ou 'ponto negativo' — use sempre 'talento para desenvolver'.\n"
        "- Ao relacionar o talento ao trabalho, adapte o exemplo ao cargo/área informado; se não houver cargo, use exemplos genéricos de operação, produção, backoffice ou liderança — não presuma que a pessoa trabalha só com eventos ou contato direto com cliente.\n"
        "- Linguagem humana, profissional, clara e positiva sem ser artificialmente elogiosa. Evite texto de aparência genérica ou automática.\n"
        "- Analise a COMBINAÇÃO dos talentos (não repita definições isoladas nos campos de combinação).\n\n"
        "Retorne APENAS um objeto JSON válido, sem texto antes ou depois, sem markdown, com exatamente esta estrutura:\n"
        "{\n"
        '  "frase_sintese": "uma frase curta sobre a característica central do perfil",\n'
        '  "talentos_predominantes": [\n'
        '    {"talento": "nome exato", "definicao_curta": "frase curta pro card", '
        '"o_que_representa": "explicação clara do talento", '
        '"como_aparece_comportamento": "como influencia pensar/agir/decidir", '
        '"como_aparece_trabalho": "aplicação no contexto de trabalho informado", '
        '"quando_bem_utilizado": "uso positivo dessa força", '
        '"ponto_atencao": "risco de excesso, sem julgamento"}, ... (exatamente 5 objetos, um por talento predominante, na ordem dada)\n'
        "  ],\n"
        '  "combinacao_talentos": "análise de como os 5 talentos predominantes funcionam JUNTOS — complementos, potencializações e tensões",\n'
        '  "visao_geral_perfil": "síntese integrada de como a pessoa tende a pensar, agir, se relacionar, decidir, reagir a pressão e onde gera mais valor",\n'
        '  "talentos_desenvolver": [\n'
        '    {"talento": "nome exato", "o_que_representa": "explicação do talento", '
        '"o_que_pode_acrescentar": "o que desenvolver isso soma ao perfil", '
        '"como_menor_evidencia_aparece": "usando \'pode acontecer de...\', sem afirmar como certeza", '
        '"onde_pode_ser_importante": "contexto de trabalho em que esse talento é exigido", '
        '"como_comecar_desenvolver": "comportamento prático e simples"}, ... (exatamente 5 objetos, um por talento a desenvolver, na ordem dada)\n'
        "  ],\n"
        '  "combinacao_gaps": "análise de como os 5 talentos a desenvolver, em conjunto, podem ampliar e complementar os talentos predominantes",\n'
        '  "direcionamento": {"continue_potencializando": ["3 a 5 itens"], "fique_atento": ["3 a 5 itens"], "desenvolva": ["3 a 5 itens"], "experimente_na_pratica": ["1 a 3 ações práticas simples"]},\n'
        '  "sintese_final": "texto curto e humanizado respondendo quem é essa pessoa profissionalmente a partir da combinação dos talentos, terminando com uma frase de incentivo não clichê"\n'
        "}"
    )
    try:
        resp = requests.post(
    ...
    if narrativa:
        ...
        for dados_talento in narrativa["talentos_predominantes"]:
            with st.expander(dados_talento["talento"]):
                st.markdown(f"**O que esse talento representa**")
                st.write(dados_talento["o_que_representa"])
                st.markdown("**Como aparece no seu comportamento**")
                st.write(dados_talento["como_aparece_comportamento"])
                st.markdown("**Como pode aparecer no trabalho**")
                st.write(dados_talento["como_aparece_trabalho"])
                st.markdown("**Quando essa força está bem utilizada**")
                st.write(dados_talento["quando_bem_utilizado"])
                st.markdown("**Ponto de atenção**")
                st.write(dados_talento["ponto_atencao"])
        ...
        # ---- 5. COMO VOCÊ TENDE A AGIR -----------------------------------
        st.markdown("##### Como Você Tende a Agir")
        st.write(narrativa["visao_geral_perfil"])
    ...
    if narrativa:
        ...
        for dados_gap in narrativa["talentos_desenvolver"]:
            with st.expander(dados_gap["talento"]):
                st.markdown("**O que esse talento representa**")
                st.write(dados_gap["o_que_representa"])
                st.markdown("**O que ele pode acrescentar ao seu perfil**")
                st.write(dados_gap["o_que_pode_acrescentar"])
                st.markdown("**Como essa menor evidência pode aparecer**")
                st.write(dados_gap["como_menor_evidencia_aparece"])
                st.markdown("**Onde esse talento pode ser importante no trabalho**")
                st.write(dados_gap["onde_pode_ser_importante"])
                st.markdown("**Como começar a desenvolver**")
                st.write(dados_gap["como_comecar_desenvolver"])
        ...
        # ---- 9. SEU DIRECIONAMENTO DE DESENVOLVIMENTO --------------------
        st.markdown("##### Seu Direcionamento de Desenvolvimento")
        direcionamento = narrativa["direcionamento"]
        col_pot, col_atencao = st.columns(2)
        with col_pot:
            st.markdown("**Continue potencializando**")
            for i in direcionamento["continue_potencializando"]:
                st.write(f"- {i}")
        with col_atencao:
            st.markdown("**Fique atento**")
            for i in direcionamento["fique_atento"]:
                st.write(f"- {i}")
        col_dev, col_pratica = st.columns(2)
        with col_dev:
            st.markdown("**Desenvolva**")
            for i in direcionamento["desenvolva"]:
                st.write(f"- {i}")
        with col_pratica:
            st.markdown("**Experimente na prática**")
            for i in direcionamento["experimente_na_pratica"]:
                st.write(f"- {i}")

        # ---- 10. SÍNTESE FINAL DO PERFIL ---------------------------------
        st.divider()
        st.markdown("##### Síntese Final do Perfil")
        st.info(narrativa["sintese_final"])
    elif narrativa_bruta:
        # Laudo gravado num formato anterior (texto corrido) — mantém a
        # leitura possível, mas sinaliza que dá pra atualizar pra nova
        # experiência clicando em "Regenerar Laudo com IA" logo abaixo.
        st.divider()
