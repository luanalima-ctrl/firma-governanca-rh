            ...
            if not colabs:
                ...
                for c in colabs:
                    with st.expander(f"{c['nome']} — {c['cargo']} ({NOMES_AREAS.get(c['area'], c['area'])})"):
                        ...
                        if pode_cadastrar_colaborador():
                            with st.popover("Editar Documentos"):
                                st.caption("Nome do documento (obrigatório) + link opcional (ex.: Google Drive).")
                                docs_atuais = c.get("documentos", [])
                                novos_documentos = []
                                for i in range(5):
                                    nome_atual = docs_atuais[i]["nome"] if i < len(docs_atuais) else ""
                                    link_atual = docs_atuais[i].get("link", "") if i < len(docs_atuais) else ""
                                    col_doc_nome, col_doc_link = st.columns(2)
                                    with col_doc_nome:
                                        nome_doc = st.text_input(f"Documento #{i+1}", value=nome_atual, key=f"doc_nome_{c['id']}_{i}")
                                    with col_doc_link:
                                        link_doc = st.text_input("Link (opcional)", value=link_atual, key=f"doc_link_{c['id']}_{i}")
                                    if nome_doc.strip():
                                        novos_documentos.append({"nome": nome_doc.strip(), "link": link_doc.strip()})
                                if st.button("Vincular Documento", key=f"salvar_documentos_{c['id']}"):
                                    atualizar_documentos(c["id"], novos_documentos)
...
with abas[IDX["alcadas"]]:
    ...
    st.caption("Régua de maturidade e gatilho financeiro da Firma.")
    ...
    if not colaborador_diretrizes:
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("Nome Completo", colaborador_diretrizes["nome"])
        c2.metric("Setor / Área", NOMES_AREAS.get(colaborador_diretrizes["area"], colaborador_diretrizes["area"]))
        c3.metric("Função / Nível Atual", colaborador_diretrizes["cargo"])

        st.divider()
        st.markdown("#### Transparência de Performance")
        st.write(
            f"Régua de Maturidade: a Nota {NOTA_REGUA_MATURIDADE:.1f} é a meta padrão da Firma e "
            "representa que o profissional está performando com total consistência e "
            "estabilidade no nível atual."
        )
        st.write(
            f"O Gatilho dos {NOTA_MINIMA_ELEGIVEL/5*100:.0f}%: a estabilidade de recebimento do "
            f"valor mensal está atrelada à manutenção da Nota Consolidada final igual ou maior "
            f"que {NOTA_MINIMA_ELEGIVEL:.1f}, representando o alcance de no mínimo "
            f"{NOTA_MINIMA_ELEGIVEL/5*100:.0f}% dos resultados esperados pela Firma."
        )
...
if papel == "RH":
    with abas[IDX["admin"]]:
        st.subheader("Gestão de Talentos Gallup")
        st.caption("Cadastro exclusivo do RH/CEO — Heads e Produtores não têm acesso a esta seção.")

        colabs_gallup = listar_colaboradores()
        if not colabs_gallup:
            st.info("Cadastre um colaborador na aba Perfil do Profissional antes de lançar o Gallup.")
        else:
            labels_gallup = lista_colaboradores_labels(colabs_gallup)
            colaborador_gallup_id = st.selectbox(
                "Colaborador", options=list(labels_gallup.keys()), format_func=lambda i: labels_gallup[i], key="admin_gallup_colab"
            )
            colaborador_gallup = obter_colaborador(colaborador_gallup_id)
            with st.form("form_editar_gallup_admin"):
                novo_gallup_admin = []
                for i in range(5):
                    valor_atual = colaborador_gallup["gallup_top5"][i] if i < len(colaborador_gallup["gallup_top5"]) else ""
                    novo_gallup_admin.append(st.text_input(f"Talento Gallup #{i+1}", value=valor_atual, key=f"admin_gallup_{i}"))
                if st.form_submit_button("Salvar Talentos Gallup"):
                    atualizar_gallup(colaborador_gallup_id, [g.strip() for g in novo_gallup_admin if g.strip()])
                    st.success(f"Talentos Gallup de {colaborador_gallup['nome']} atualizados.")
                    st.rerun()
