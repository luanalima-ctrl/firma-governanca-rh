...
if papel == "RH":
    with abas[IDX["admin"]]:
        ...
        if not modo_criacao_prontuario:
            st.divider()
            st.markdown("##### ⚠️ Zona de Perigo — Exclusão Definitiva")
            st.caption("Esta ação é destrutiva e irreversível. Ela apagará o colaborador e TODO o histórico de avaliações técnicas e mensais atrelados a ele para sempre.")

            usuario_vinculado = usuario_vinculado_a_colaborador(selecao_prontuario)
            if usuario_vinculado:
                st.warning(
                    ...
                    "Recomendo remover esse usuário primeiro, na seção 'Remover usuário' logo abaixo."
                )

            confirmar_exclusao_total = st.checkbox(
                f"Confirmo que desejo APAGAR DEFINITIVAMENTE o registro de {dados_existentes_prontuario['nome']} e todas as suas avaliações do sistema.",
                key=f"pront_excluir_total_confirma_{selecao_prontuario}"
            )

            if st.button("Excluir Colaborador Permanentemente", key=f"pront_excluir_total_btn_{selecao_prontuario}", disabled=not confirmar_exclusao_total, type="secondary"):
                excluir_colaborador_definitivo(selecao_prontuario)
                st.success(f"O colaborador foi removido permanentemente do sistema.")
                st.rerun()
        ...
        st.divider()
        st.markdown("##### Excluir Avaliações Lançadas")
        st.caption("Ação irreversível — exclusivo do RH/CEO. Use apenas para corrigir lançamentos feitos por engano.")
        ...
        if not colabs_para_excluir_aval:
            ...
            colaborador_aval_id = st.selectbox(
                "Colaborador", options=list(mapa_colabs_aval.keys()), format_func=lambda i: mapa_colabs_aval[i],
                key="admin_excluir_aval_colab",
            )
            confirmar_exclusao_aval = st.checkbox(
                "Confirmo que quero excluir avaliações deste colaborador (ação irreversível)",
