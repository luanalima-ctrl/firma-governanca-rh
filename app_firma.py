...
def _narrativa_comportamental_offline(dominio_principal: str, forcas: str, fraquezas: str, top_talentos: list, pontos_desenvolvimento: list) -> str:
    """Fallback offline com a mesma estrutura de 5 seções exigida do modo
    online, para a devolutiva nunca ficar rasa quando a API não estiver
    configurada ou falhar."""
    return (
        "### SÍNTESE DO PERFIL E DINÂMICA COMPORTAMENTAL\n"
        f"Seu perfil demonstra uma forte inclinação para o Domínio de {dominio_principal}, o que molda a forma como "
        "você se organiza diante de demandas, prioriza o que precisa ser feito e se posiciona diante de imprevistos "
        "— seja atuando na fita técnica de eventos ou no backoffice da Firma. Esse domínio tende a definir seu ritmo "
        "natural de trabalho e a maneira como você interage com a equipe e com o cliente no dia a dia.\n\n"
        "### RAIO-X DAS HABILIDADES E FORÇAS INDIVIDUAIS\n"
        f"O conjunto formado por {forcas} funciona de maneira integrada no seu dia a dia: são competências que se "
        "reforçam mutuamente e, juntas, explicam boa parte da sua capacidade natural de gerar resultado sem exigir "
        "esforço consciente para sustentar esse comportamento. É provável que essas forças apareçam combinadas nas "
        "situações em que você se sente mais confiante e produtivo no trabalho.\n\n"
        "### FATORES DE DESGASTE E RISCO (PONTOS CEGOS)\n"
        f"Como toda força, suas competências de destaque ({forcas}) podem se tornar um ponto cego quando usadas em "
        "excesso ou sob forte pressão — a mesma característica que gera valor no dia a dia pode ser percebida pela "
        "equipe ou pelo cliente como rigidez, impaciência ou distanciamento quando você está no limite.\n\n"
        "### DIRETRIZES PARA DESENVOLVIMENTO DE GAPS\n"
        f"Os itens {fraquezas}, combinados, indicam que essa não é uma tendência natural do seu perfil — o que não "
        "significa incapacidade, apenas que exige mais atenção consciente. Nos próximos 30 dias, vale observar "
        "situações em que essas competências são exigidas e praticar deliberadamente uma resposta diferente da sua "
        "tendência natural, buscando equilibrar esses gaps de forma gradual.\n\n"
        "### ESTILO DE GESTÃO REQUERIDO (DIRETRIZ PARA O LÍDER/HEAD)\n"
        "Para extrair o melhor deste perfil, recomenda-se que o gestor direto ofereça clareza de expectativas, "
        "feedback direto e objetivo, e reconhecimento explícito quando as forças naturais do colaborador forem bem "
        "aplicadas — evitando microgestão excessiva sobre as frentes em que ele já demonstra domínio consistente."
    )


def gerar_narrativa_comportamental(dominio_principal: str, top_talentos: list, pontos_desenvolvimento: list) -> str:
    """Chama a API do Claude para gerar um laudo comportamental estendido
    em 5 seções estruturadas (Analista de RH Sênior), com fallback offline
    na mesma estrutura caso a chave não esteja configurada ou a chamada falhe."""
    forcas = ", ".join(t["talento"] for t in top_talentos)
    fraquezas = ", ".join(pontos_desenvolvimento)

    api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not api_key:
        return _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas, top_talentos, pontos_desenvolvimento)

    prompt_profiler_senior = (
        "Você é um Analista Comportamental Sênior e Business Partner de RH na Firma Produções. "
        "Sua missão é redigir uma Devolutiva de Perfil Comportamental de alto padrão, técnica, densa e transformadora para o colaborador, "
        "com base nos dados puramente matemáticos extraídos do nosso inventário de 30 situações.\n\n"
        "[DADOS TÉCNICOS DO COLABORADOR]\n"
        f"- Domínio Comportamental Predominante: {dominio_principal}\n"
        f"- Top 5 Talentos mais Evidentes: {forcas}\n"
        f"- Pontos de Desenvolvimento Menos Evidentes: {fraquezas}\n\n"
        "[DIRETRIZES OBRIGATÓRIAS DE REDAÇÃO E ESTRUTURA]\n"
        "Escreva em segunda pessoa ('Você'). Mantenha um tom altamente profissional, clínico e executivo de RH. "
        "Não use listas de tópicos simples com uma linha. Desenvolva explicações densas e robustas (mínimo de 3 a 4 linhas por pilar/tópico). "
        "Sua resposta deve conter EXATAMENTE as seguintes 5 seções textuais, separadas por títulos em negrito (use '###' antes de cada título, sem emojis):\n\n"
        "### 1. SÍNTESE DO PERFIL E DINÂMICA COMPORTAMENTAL\n"
        "Escreva um parágrafo robusto analisando a essência do profissional com base no seu Domínio Predominante. Explique como ele reage sob pressão, seu ritmo de trabalho, senso de urgência e sua forma de interagir com o meio (seja no backoffice ou nas produções em campo).\n\n"
        "### 2. RAIO-X DAS HABILIDADES E FORÇAS INDIVIDUAIS\n"
        "Analise o bloco das 5 principais forças evidentes. Explique detalhadamente o significado prático dessa combinação de talentos na rotina de trabalho e de que forma o colaborador gera resultados através deles de forma integrada.\n\n"
        "### 3. FATORES DE DESGASTE E RISCO (DESVANTAGENS DO PERFIL)\n"
        "Explique como o excesso ou mau uso das suas forças pode se transformar em um ponto cego (sombra do perfil). Descreva de que forma o comportamento dele pode ser interpretado pela equipe ou clientes quando ele está sob forte estresse ou focado demais em seus próprios objetivos.\n\n"
        "### 4. DIRETRIZES PARA DESENVOLVIMENTO DE GAPS\n"
        "Analise os itens listados como Talentos para Desenvolver. Explique o que a baixa pontuação combinada significa comportamentalmente e dê uma diretriz atitudinal e prática para o colaborador começar a equilibrar esses gaps nos próximos 30 dias.\n\n"
        "### 5. ESTILO DE GESTÃO REQUERIDO (DIRETRIZ PARA O LÍDER/HEAD)\n"
        "Escreva um parágrafo direcionado ao Head de Área/Gestor do colaborador. Explique quais são os Fatores Motivacionais e as Necessidades Básicas deste perfil (ex: autonomia, reconhecimento, rotinas claras ou desafios), indicando exatamente como o líder deve conduzir os feedbacks e delegar tarefas para extrair o potencial máximo deste profissional sem causar desmotivação ou afastamento.\n\n"
        "Retorne apenas o texto clínico final estruturado nessas 5 seções. Não adicione saudações, notas explicativas, introduções ou caracteres markdown fora dos cabeçalhos das seções."
    )
    try:
        resp = requests.post(
            ANTHROPIC_API_URL,
            ...
            json={
                "model": ANTHROPIC_MODEL,
                "max_tokens": 3000,
                "messages": [{"role": "user", "content": prompt_profiler_senior}],
            },
            timeout=45,
        )
        resp.raise_for_status()
        ...
        return texto or _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas, top_talentos, pontos_desenvolvimento)
    except Exception:
        return _narrativa_comportamental_offline(dominio_principal, forcas, fraquezas, top_talentos, pontos_desenvolvimento)
...
def renderizar_teste_comportamental(colaborador_id: str):
...
def renderizar_resultado_comportamental(perfil: dict, titulo: str = "Perfil Comportamental", colaborador_id: Optional[str] = None):
    """Exibição sóbria e corporativa do resultado já processado. Quando
    colaborador_id é informado, mostra um botão para regenerar só o texto
    do laudo (sem exigir que a pessoa refaça as 30 perguntas) — útil para
    laudos gerados antes de uma melhoria no prompt da IA."""
    st.markdown(f"#### {titulo}")
    st.info(f"**Domínio de Maior Destaque:** {perfil['dominio_principal']}")

    col_top, col_dev = st.columns(2)
    with col_top:
        st.markdown("**Principais Talentos**")
        for item in perfil["top_talentos"]:
            st.write(f"- {item['talento']}")
    with col_dev:
        st.markdown("**Talentos para Desenvolver**")
        for talento in perfil["pontos_desenvolvimento"]:
            st.write(f"- {talento}")

    if perfil.get("narrativa_ia"):
        st.divider()
        st.markdown("##### Leitura do seu Perfil")
        st.markdown(perfil["narrativa_ia"])
        ...
        if st.button("Regenerar Laudo com IA", key=f"regenerar_laudo_{colaborador_id}"):
            with st.spinner("Gerando uma nova leitura do perfil..."):
                ...
                salvar_perfil_comportamental(
                    ...
                    perfil["pontos_desenvolvimento"], nova_narrativa,
                )
    ...
    if papel == "COLABORADOR":
        ...
        if not colaborador_proprio:
            ...
            if perfil_comportamental_proprio:
                renderizar_resultado_comportamental(perfil_comportamental_proprio, "Seu Perfil Comportamental", colaborador_proprio["id"])
            else:
                renderizar_teste_comportamental(colaborador_proprio["id"])
            ...
            for c in colabs:
                with st.expander(f"{c['nome']} — {c['cargo']} ({NOMES_AREAS.get(c['area'], c['area'])})"):
                    ...
                    if perfil_comportamental_c:
                        renderizar_resultado_comportamental(perfil_comportamental_c, "Perfil Comportamental", c["id"])
                    else:
