    ...
    if papel == "COLABORADOR":
        ...
        if not colaborador_proprio:
            ...
            st.markdown(
                ...
                f"**Tempo de empresa:** {meses_entre(colaborador_proprio['data_admissao'])} mês(es)",
                unsafe_allow_html=True,
            )
    ...
    with abas[IDX["historico"]]:
        ...
        if not colabs:
            ...
            exibir_tabela(pd.DataFrame([
                {
                    "Tempo de empresa": f"{eleg.tempo_empresa_meses} mês(es)",
                    "Tempo no nível atual": f"{eleg.tempo_nivel_meses} mês(es)",
