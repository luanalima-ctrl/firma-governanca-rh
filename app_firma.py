...
# mês (todos os pós-eventos + a Ficha Mensal + o Perfil Comportamental) e
# gera um único plano de desenvolvimento unificado.
# ---------------------------------------------------------------------------

def montar_compilado_mensal(ficha_mensal, campo_periodo: list) -> dict:
    """Consolida todos os pós-eventos do mês com a Ficha Mensal, extraindo
...
PDI_ACOMPANHAMENTO_FIXO = {
    ...
    "fontes": [
        ...
        "Avaliação técnica / pós-evento",
        "Feedback da liderança direta",
...
def _validar_plano_mensal(parsed: dict) -> bool:
    ...
    desenvolvimento, 1 a 3 ações) e os campos de cada linha das tabelas."""
    try:
        ...
        for a in parsed["plano_acao"]:
            if not all(k in a for k in ("desenvolvimento", "acao_pratica", "aplicacao", "acompanhamento")):
                return False
        if not isinstance(parsed["indicadores_evolucao"], list) or not parsed["indicadores_evolucao"]:
            return False
        return True
...
def _plano_mensal_offline(compilado: dict, nome_colab: str = "o colaborador") -> dict:
    ...
    esperado, sem inventar causa, intenção ou comportamento."""
    ...
    if campo:
        pontos_desenvolvimento.append({
            "ponto": "Entrega técnica em campo",
            "o_que_foi_identificado": (
                f"As avaliações técnicas do período registraram resultado abaixo do esperado em: "
                f"{', '.join(campo[:4])}."
            ),
            "o_que_precisa_mudar": (
                "Cumprir os critérios técnicos da função dentro do padrão esperado nos próximos eventos, "
                "com atenção específica aos itens acima."
            ),
        })
    if parte_a:
        pontos_desenvolvimento.append({
            ...
            "o_que_foi_identificado": (
                f"A avaliação mensal registrou resultado abaixo do esperado em: {', '.join(parte_a[:4])}."
            ),
            "o_que_precisa_mudar": (
                "Demonstrar, nas atividades do dia a dia, o nível de entrega esperado para o cargo atual "
                "nos critérios apontados."
            ),
        })
    if parte_b:
        pontos_desenvolvimento.append({
            ...
            "o_que_foi_identificado": (
                f"A avaliação de cultura e valores registrou resultado abaixo do esperado em: "
                f"{', '.join(parte_b[:4])}."
            ),
            "o_que_precisa_mudar": (
                "Demonstrar de forma prática, no ambiente de trabalho, os comportamentos associados "
                "a esses valores."
            ),
        })
    ...
    plano_acao = [
        {
            ...
            "acao_pratica": "Alinhar com a liderança direta o padrão esperado e aplicá-lo nas atividades do período.",
            "aplicacao": "Durante as atividades e eventos do próximo ciclo.",
            "acompanhamento": "Avaliação mensal e avaliações técnicas do próximo período.",
        }
    ...
    return {
        "resultado_periodo": (
            f"No período avaliado, a média de entrega técnica em campo foi de "
            f"{compilado['media_campo_mes']:.2f}/5,0. O resultado consolidado ficou abaixo do mínimo "
            f"de {NOTA_MINIMA_ELEGIVEL:.1f} exigido para elegibilidade, o que motiva a abertura deste plano."
        ),
        "pontos_positivos": [],
        "pontos_desenvolvimento": pontos_desenvolvimento[:3],
        "plano_acao": plano_acao,
        "indicadores_evolucao": [
            "Evolução da nota nos critérios apontados como pontos de desenvolvimento.",
            "Redução de ocorrências registradas nas avaliações técnicas dos próximos eventos.",
            f"Nota consolidada do próximo período igual ou superior a {NOTA_MINIMA_ELEGIVEL:.1f}.",
        ],
        "expectativa_proximo_ciclo": (
            "No próximo ciclo, espera-se evolução nos critérios apontados acima. Esses pontos serão "
            "acompanhados nas avaliações mensais e nas avaliações técnicas dos próximos eventos."
        ),
    }
...
def gerar_plano_desenvolvimento_mensal(periodo_sel: str, ficha_mensal, compilado: dict, perfil, nome_colab: str = "", cargo_colab: str = "") -> dict:
    ...
    if not (os.environ.get("ANTHROPIC_API_KEY", "").strip()):
        ...
        return _plano_mensal_offline(compilado, nome_colab)
    ...
    prompt_rh_senior = (
        ...
        "transformar os resultados das avaliações de desempenho em um Plano de Desenvolvimento Individual (PDI) "
        "claro, objetivo, transparente e aplicável. O documento será lido pelo PRÓPRIO COLABORADOR.\n\n"
        "Sua função NÃO é fazer análise psicológica, julgar o colaborador ou criar narrativa sobre a "
        "personalidade dele. Você responde apenas: como ele está performando, onde precisa melhorar, o que "
        "precisa fazer diferente e como a evolução será acompanhada.\n\n"
        f"[DADOS DO PERÍODO — {periodo_sel}]\n"
        f"- Colaborador: {nome_colab}\n"
        f"- Cargo / Nível: {cargo_colab or 'não informado'}\n"
        f"- Média de entrega técnica (avaliações pós-evento): {compilado['media_campo_mes']:.2f}/5,0\n"
        f"- Critérios técnicos com nota 1 ou 2: {compilado['kpis_baixos_campo'] or 'nenhum'}\n"
        f"- Comentários da liderança nas avaliações técnicas: {comentarios_campo}\n"
        f"- Nota de desempenho e governança (Parte A): {f'{media_parte_a:.2f}/5,0' if media_parte_a is not None else 'sem ficha mensal no período'}\n"
        f"- Critérios de governança com nota 1 ou 2: {compilado['kpis_baixos_parte_a'] or 'nenhum'}\n"
        f"- Nota de cultura e valores (Parte B): {f'{media_parte_b:.2f}/5,0' if media_parte_b is not None else 'sem ficha mensal no período'}\n"
        f"- Valores com nota 1 ou 2: {compilado['kpis_baixos_parte_b'] or 'nenhum'}\n"
        f"- Comentários da liderança na ficha mensal: {comentarios_mensal}\n\n"
        f"[PERFIL COMPORTAMENTAL]\n{bloco_perfil}\n\n"
        "[REGRAS OBRIGATÓRIAS]\n"
        "1. LÓGICA: nota → evidência → ponto de desenvolvimento → ação → indicador. Toda recomendação deve "
        ...
        "2. FIDELIDADE: se não houver evidência suficiente para concluir algo, escreva apenas o que os dados "
        "permitem afirmar. Nunca atribua intenção, escolha, caráter, emoção ou motivação ao colaborador.\n"
        "3. TOM: direto, claro, respeitoso, brando, transparente e construtivo. O colaborador deve entender o "
        "documento sem precisar de interpretação do RH. Avalie comportamentos e entregas, nunca caráter.\n"
        "4. PROIBIDO usar expressões como 'subutilização consciente do potencial', 'foi uma escolha', 'optou "
        "pelo silêncio', 'falta de responsabilidade', 'desconexão com a cultura', 'comportamento preocupante', "
        "'ausência de comprometimento'. PREFIRA: 'o resultado indica necessidade de evolução em...', 'foi "
        "identificado um ponto de atenção em...', 'para o próximo ciclo, espera-se...'.\n"
        "5. CULTURA E VALORES: nunca trate como julgamento de identidade ou caráter. Transforme cada valor em "
        "comportamento observável (ex.: Honestidade → comunicar erros e dificuldades à liderança).\n"
        "6. GOVERNANÇA: relacione o resultado às responsabilidades esperadas para o NÍVEL ATUAL do cargo. Não "
        "compare o colaborador com níveis superiores.\n"
        "7. PRIORIZAÇÃO: o PDI deve ser executável. Selecione no máximo 3 pontos de desenvolvimento, "
        "priorizando o que mais impacta a entrega, o que é recorrente e o que pode ser desenvolvido no "
        "próximo ciclo. Não liste tudo que poderia melhorar.\n"
        "8. AÇÕES: de 1 a 3, realistas para o cargo e nível informados.\n"
        "9. INDICADORES: simples, ligados aos pontos identificados e preferencialmente já existentes nas "
        "avaliações da Firma. Evite números artificiais ('duas iniciativas', '100%') sem relação com o "
        "processo de avaliação.\n"
        "10. PONTOS POSITIVOS: no máximo 3, e apenas os que realmente aparecem nos resultados. Se não houver "
        "evidência de ponto positivo nos dados, devolva uma lista vazia. Não elogie genericamente.\n\n"
        "Retorne APENAS um objeto JSON válido, sem texto antes ou depois, sem markdown, com exatamente esta estrutura:\n"
        "{\n"
        '  "resultado_periodo": "síntese objetiva do período em 1 a 2 parágrafos: nota/média geral, principais resultados, principais pontos de atenção e evoluções relevantes se houver. Sem introdução motivacional.",\n'
        '  "pontos_positivos": [{"titulo": "nome curto", "descricao": "o que os resultados mostram"}],\n'
        '  "pontos_desenvolvimento": [{"ponto": "nome claro e objetivo", "o_que_foi_identificado": "o resultado com base nas notas e evidências disponíveis", "o_que_precisa_mudar": "o comportamento ou entrega esperada"}],\n'
        '  "plano_acao": [{"desenvolvimento": "o que precisa ser desenvolvido", "acao_pratica": "o que o colaborador deverá fazer", "aplicacao": "em qual situação isso acontece", "acompanhamento": "como a liderança/RH verificará"}],\n'
        '  "indicadores_evolucao": ["indicador 1", "indicador 2", "..."],\n'
        '  "expectativa_proximo_ciclo": "orientação curta e objetiva sobre o que a liderança espera ver diferente no próximo ciclo. Sem discurso motivacional."\n'
        "}"
    )
    try:
        resp = requests.post(
...
def abas_visiveis_para_papel() -> list[str]:
    ...
    if papel == "HEAD":
        # Remove a aba de campo pós-evento do escopo de qualquer Head de Área cadastrado
        ...
        if not colabs_para_excluir_aval:
            ...
            with st.expander(f"Avaliações de Entrega Técnica ({len(avaliacoes_campo_do_colab)})"):
                if not avaliacoes_campo_do_colab:
                    st.caption("Nenhuma avaliação pós-evento lançada para este colaborador.")
