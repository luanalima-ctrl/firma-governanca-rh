...
def coletar_criterios_baixa_nota(
    ...
    recebeu nota 1 ou 2, no formato exigido pelo gerador de PDI:
        {"pilar": ..., "nota": ..., "criterio_oficial": ..., "fato_gerador": ...,
         "colaborador_id": ..., "periodo": ...}
    colaborador_id/periodo são carimbados em cada achado para que o gerador
    de PDI consiga cruzar com a Ficha Mensal e o Perfil Comportamental na
    hora de redigir o plano de ação.
    """
    achados = []
...
def _gerar_acao_online(pilar: str, criterio_oficial: str, fato_gerador: str, api_key: str, contexto_evolucao: str = "") -> Optional[dict]:
...
def _montar_contexto_evolucao(colaborador_id: Optional[str], periodo: Optional[str]) -> str:
    """Busca a última Ficha Mensal e o Perfil Comportamental do colaborador
    (quando disponíveis) e monta o bloco de contexto injetado no prompt do
    PDI, para a IA cruzar o erro pontual com o histórico de desempenho,
    cultura e comportamento — sem inventar procedimentos técnicos de campo."""
    if not colaborador_id:
        return ""

    ficha_mensal = obter_avaliacao_mensal(colaborador_id, periodo) if periodo else None
    perfil = obter_perfil_comportamental(colaborador_id)

    if not ficha_mensal and not perfil:
        return ""

    contexto_evolucao = "\n\n[HISTÓRICO DE DESEMPENHO E COMPORTAMENTO DO COLABORADOR]"

    if perfil:
        forcas = ", ".join(t["talento"] for t in perfil["top_talentos"])
        contexto_evolucao += (
            f"\n- Perfil Comportamental (Domínio Principal): {perfil['dominio_principal']}\n"
            f"- Suas Maiores Forças Naturais: {forcas}"
        )

    if ficha_mensal:
        contexto_evolucao += (
            f"\n- Média Geral de Desempenho (Parte A) no mês: {ficha_mensal['media_parte_a']:.2f}/5.0\n"
            f"- Média Geral de Cultura e Valores (Parte B) no mês: {ficha_mensal['media_parte_b']:.2f}/5.0"
        )

    contexto_evolucao += (
        "\n\nINSTRUÇÃO DE FEEDBACK: Ao redigir as ações do PDI ('o_que_fazer' e 'como_medir'), "
        "cruze o erro atual do critério avaliado com este histórico. Oriente o colaborador de forma ampla "
        "sobre como equilibrar suas competências mensais e engajamento com a cultura da Firma para mitigar "
        "novas falhas de atitude ou entrega, mantendo o tom estritamente focado em diretrizes corporativas, "
        "sem inventar ou detalhar procedimentos de engenharia técnica de campo."
    )
    return contexto_evolucao


def gerar_plano_de_acao(achado: dict) -> dict:
    """Decide entre modo online/offline e devolve a ação para um achado.
    Quando online, cruza o achado com a última Ficha Mensal e o Perfil
    Comportamental do colaborador (se já existirem no banco) para dar à IA
    contexto de evolução, não só o erro pontual isolado."""
    api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        contexto_evolucao = _montar_contexto_evolucao(achado.get("colaborador_id"), achado.get("periodo"))
        resultado = _gerar_acao_online(
            achado["pilar"], achado["criterio_oficial"], achado["fato_gerador"], api_key, contexto_evolucao
        )
        if resultado:
            return resultado
