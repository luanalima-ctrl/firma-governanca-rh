...
def _narrativa_comportamental_offline(dominio_principal: str, forcas: str, fraquezas: str, top_talentos: list, pontos_desenvolvimento: list) -> str:
    ...
    topicos_forcas = "\n\n".join(
        f"**{t['talento']}**\n"
        f"Este talento aparece entre os mais evidentes do seu perfil. Na rotina de trabalho, ele se manifesta como uma "
        f"disposição natural para atuar de forma consistente nessa frente, gerando resultado sem precisar de esforço "
        f"consciente para sustentar esse comportamento."
        for t in top_talentos
    )
    topicos_gaps = "\n\n".join(
        f"**{g}**\n"
        f"Uma pontuação mais baixa aqui indica que essa não é uma tendência natural do seu perfil — o que não significa "
        f"incapacidade, apenas que exige mais atenção consciente. Nos próximos 30 dias, vale observar situações em que "
        f"essa competência é exigida e praticar deliberadamente uma resposta diferente da sua tendência natural."
        for g in pontos_desenvolvimento
    )
    return (
        ...
        "### RAIO-X DAS HABILIDADES E FORÇAS INDIVIDUAIS (TOP 5)\n"
        f"{topicos_forcas}\n\n"
        "### FATORES DE DESGASTE E RISCO (DESVANTAGENS DO PERFIL)\n"
        ...
        "### MAPA DE GAPS E PONTOS DE DESENVOLVIMENTO\n"
        f"{topicos_gaps}\n\n"
...
def gerar_narrativa_comportamental(dominio_principal: str, top_talentos: list, pontos_desenvolvimento: list) -> str:
    ...
    prompt_profiler_senior = (
        ...
        "### 2. RAIO-X DAS HABILIDADES E FORÇAS INDIVIDUAIS (TOP 5)\n"
        "Para CADA UM dos 5 talentos listados no Top 5, abra um subtópico em negrito e explique detalhadamente o significado prático dele. Descreva qual a vantagem desse talento, como ele se manifesta na rotina de trabalho e de que forma o colaborador gera resultados através dele.\n\n"
        ...
        "### 4. MAPA DE GAPS E PONTOS DE DESENVOLVIMENTO\n"
        "Para CADA UM dos itens listados como Pontos de Desenvolvimento, abra um subtópico em negrito e explique o que a baixa pontuação significa comportamentalmente. Mostre o que está faltando nessa competência e dê uma diretriz atitudinal e prática para o colaborador começar a desenvolver esse gap nos próximos 30 dias.\n\n"
...
def renderizar_resultado_comportamental(perfil: dict, titulo: str = "Perfil Comportamental"):
    """Exibição sóbria e corporativa do resultado já processado."""
    ...
    if papel == "COLABORADOR":
        ...
        if not colaborador_proprio:
            ...
            if perfil_comportamental_proprio:
                renderizar_resultado_comportamental(perfil_comportamental_proprio, "Seu Perfil Comportamental")
            else:
            ...
            for c in colabs:
                with st.expander(f"{c['nome']} — {c['cargo']} ({NOMES_AREAS.get(c['area'], c['area'])})"):
                    ...
                    if perfil_comportamental_c:
                        renderizar_resultado_comportamental(perfil_comportamental_c, "Perfil Comportamental")
                    else:
