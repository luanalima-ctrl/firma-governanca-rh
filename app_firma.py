...
(arquivo único, autocontido, com login por papéis e persistência SQLite)
"""
...
# ============================================================
# SEÇÃO 2/6 — PERSISTÊNCIA SQLITE (db.py)
# ============================================================
# -*- coding: utf-8 -*-
"""
db.py
Camada de persistência em SQLite para o sistema de governança da Firma
Produções. Substitui o armazenamento em memória (st.session_state) por um
arquivo de banco de dados real (firma_rh.db), que sobrevive a reinícios do
app e a "sonos" do Streamlit Cloud.

Todas as datas são guardadas em formato ISO (YYYY-MM-DD) e listas/dicts são
serializados em JSON dentro de colunas TEXT — abordagem simples e suficiente
para o volume de dados de uma empresa com menos de 100 funcionários. Se o
volume crescer muito, trocar para Postgres reaproveitando as mesmas funções
públicas deste módulo (a interface não muda).
"""
...
# ---------------------------------------------------------------------------
# Conexão e inicialização do schema
# ---------------------------------------------------------------------------
...
def _migrar_papel_produtor(conn: sqlite3.Connection):
    """
    Migração idempotente: bancos criados pela v2 têm a tabela 'usuarios' com
    CHECK (papel IN ('RH','HEAD','COLABORADOR')), o que bloqueia o novo papel
    'PRODUTOR'. Detecta esse caso pelo SQL de criação salvo no sqlite_master
    e recria a tabela sem CHECK (validação de papel passa a ser só na
    camada Python, em PAPEIS_VALIDOS), preservando todos os dados.
    """
    row = conn.execute(
