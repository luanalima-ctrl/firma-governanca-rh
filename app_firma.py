...
def _plano_mensal_offline(compilado: dict) -> dict:
    citados = (compilado["kpis_baixos_campo"] + compilado["kpis_baixos_mensal"])[:5]
    texto_citados = ", ".join(citados) if citados else "os critérios avaliados no mês"
    return {
        "titulo": "Plano de Desenvolvimento Unificado do Mês",
        "o_que_fazer": (
            f"Revisar com o gestor direto, na próxima 1:1, os pontos de atenção identificados no mês "
            f"({texto_citados}), buscando equilibrar consistência técnica em campo com engajamento na "
            "cultura e nos valores da Firma."
        ),
        "como_medir": (
            f"Nota Consolidada do próximo período (Ficha Mensal + média de campo, peso 50/50) igual ou "
            f"superior a {NOTA_MINIMA_ELEGIVEL:.1f}."
        ),
    }


def gerar_plano_desenvolvimento_mensal(periodo_sel: str, ficha_mensal, compilado: dict, perfil) -> dict:
    """Gera o plano de desenvolvimento único do mês, cruzando o compilado
    de campo + Ficha Mensal + Perfil Comportamental num só prompt. Sempre
    devolve as 3 chaves obrigatórias (titulo, o_que_fazer, como_medir),
    ...
    if not api_key:
        return _plano_mensal_offline(compilado)
    ...
    contexto_compilado_mes = (
        f"[COMPILADO DE PERFORMANCE MENSAL - PERÍODO: {periodo_sel}]\n"
        f"- Média Geral de Entrega Técnica (Pós-Eventos do Mês): {compilado['media_campo_mes']:.2f}/5.0\n"
        f"- KPIs de Campo com nota baixa (1 ou 2) no mês: {compilado['kpis_baixos_campo']}\n"
        f"- Justificativas acumuladas dos produtores em campo: {comentarios_acumulados_campo}\n\n"
        f"- Nota de Desempenho e Governança (Ficha Mensal - Parte A): {media_parte_a:.2f}/5.0\n"
        f"- Nota de Cultura e Valores (Ficha Mensal - Parte B): {media_parte_b:.2f}/5.0\n"
        f"- Justificativas do Head na Ficha Mensal: {comentarios_mensais_baixos}\n\n"
        f"[PERFIL COMPORTAMENTAL PROPRIETÁRIO]\n"
        f"- Domínio Principal: {perfil['dominio_principal'] if perfil else 'Não realizado'}\n"
        f"- Forças Naturais: {', '.join(t['talento'] for t in perfil['top_talentos']) if perfil else '—'}\n\n"
        "INSTRUÇÃO OBRIGATÓRIA DE REDAÇÃO: Redija um plano de desenvolvimento unificado para os próximos 30 dias. "
        "Não cite projetos isolados. Faça uma análise atitudinal abrangente. Mostre como o colaborador deve "
        "equilibrar suas competências de escritório (cultura) com sua consistência em campo, usando suas forças "
        "comportamentais para mitigar os gargalos apontados no mês. Mantenha os outputs JSON de chaves intactos."
    )

    prompt = (
        "Você é um especialista em RH e desenvolvimento de pessoas em uma produtora de eventos corporativos.\n\n"
        f"{contexto_compilado_mes}\n\n"
        "Gere APENAS um objeto JSON, sem nenhum texto antes ou depois, sem markdown, com exatamente estas 3 chaves:\n"
        '{"titulo": "título curto do plano de desenvolvimento do mês", '
        '"o_que_fazer": "orientação ampla e unificada de comportamento e atitude para os próximos 30 dias", '
        '"como_medir": "métrica objetiva combinando campo e cultura para os próximos 30 dias"}'
    )
    try:
        resp = requests.post(
            ANTHROPIC_API_URL,
...
def gerar_pdf_pdi(
    ...
    # ---- Seção 1: Diagnóstico consolidado do mês -----------------------
    story.append(Paragraph("1. DIAGNÓSTICO CONSOLIDADO DO MÊS", heading_style))

    story.append(Paragraph(
        f"<b>Média de Entrega Técnica (Pós-Eventos do Mês):</b> {compilado['media_campo_mes']:.2f} / 5.0",
        body_style,
    ))
    if ficha_mensal:
        story.append(Paragraph(
            f"<b>Nota de Desempenho e Governança (Parte A):</b> {ficha_mensal['media_parte_a']:.2f} / 5.0",
            body_style,
        ))
        story.append(Paragraph(
            f"<b>Nota de Cultura e Valores (Parte B):</b> {ficha_mensal['media_parte_b']:.2f} / 5.0",
            body_style,
        ))
    story.append(Spacer(1, 8))

    tem_kpis = compilado["kpis_baixos_campo"] or compilado["kpis_baixos_mensal"]
    if not tem_kpis:
        story.append(Paragraph(
            "Nenhum critério individual com nota 1 ou 2 foi identificado, porém a Nota Consolidada do "
            "período ficou abaixo do gatilho de elegibilidade. Recomenda-se revisão geral com o "
            "colaborador na próxima reunião de 1:1.",
            body_style,
        ))
    else:
        if compilado["kpis_baixos_campo"]:
            story.append(Paragraph("<b>KPIs de Campo com nota baixa no mês:</b>", bold_style))
            for kpi in compilado["kpis_baixos_campo"]:
                story.append(Paragraph(f"— {kpi}", body_style))
            story.append(Spacer(1, 6))
        if compilado["kpis_baixos_mensal"]:
            story.append(Paragraph("<b>Critérios da Ficha Mensal com nota baixa:</b>", bold_style))
            for kpi in compilado["kpis_baixos_mensal"]:
                story.append(Paragraph(f"— {kpi}", body_style))

    # ---- Seção 2: Plano de desenvolvimento unificado --------------------
    story.append(Paragraph("2. PLANO DE DESENVOLVIMENTO UNIFICADO — PRÓXIMOS 30 DIAS", heading_style))
    story.append(Paragraph(f"<b>{plano['titulo']}</b>", bold_style))
    story.append(Paragraph(f"<i>O que fazer:</i> {plano['o_que_fazer']}", body_style))
    story.append(Paragraph(f"<i>Como medir o sucesso em 30 dias:</i> {plano['como_medir']}", body_style))
                ...
                if papel == "COLABORADOR" and not resultado["elegivel"]:
                    ...
                    if st.button("Gerar meu PDI em PDF"):
                        with st.spinner("Consolidando o mês e gerando o relatório..."):
                            ...
                            plano_proprio = gerar_plano_desenvolvimento_mensal(periodo_sel, ficha_mensal, compilado_proprio, perfil_proprio_pdi)
                ...
                if resultado["elegivel"]:
                    ...
                    if st.button("Gerar PDI Corporativo", type="primary"):
                        ...
                        with st.spinner("Consolidando o mês inteiro e gerando o relatório..."):
                            plano = gerar_plano_desenvolvimento_mensal(periodo_sel, ficha_mensal, compilado, perfil_pdi)
