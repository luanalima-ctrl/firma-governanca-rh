...
PLANO_MENSAL_CHAVES_OBRIGATORIAS = (
    ...
    "eixo1_roteiro", "eixo1_o_que_fazer", "eixo1_como_medir",
    "eixo2_roteiro", "eixo2_o_que_fazer", "eixo2_como_medir",
    "eixo3_roteiro", "eixo3_o_que_fazer", "eixo3_como_medir",
    "fechamento_1a1",
)
...
def _plano_mensal_offline(compilado: dict, nome_colab: str = "o colaborador") -> dict:
    """Fallback offline com a mesma estrutura rica (diagnóstico + 3 eixos +
    fechamento) exigida do modo online — usado quando a API não está
    configurada ou falha, para o PDF nunca ficar sem conteúdo."""
    campo_citados = ", ".join(compilado["kpis_baixos_campo"][:3]) or "os itens de entrega técnica do mês"
    parte_a_citados = ", ".join(compilado["kpis_baixos_parte_a"][:3]) or "os pilares de desempenho do mês"
    parte_b_citados = ", ".join(compilado["kpis_baixos_parte_b"][:3]) or "os valores culturais do mês"

    return {
        "diagnostico_integrado": (
            f"O fechamento do período consolidou uma nota abaixo do padrão de excelência da Firma, "
            f"combinando desvios em campo ({campo_citados}) com pontos de atenção na Ficha Mensal "
            f"({parte_a_citados}; {parte_b_citados}). Recomenda-se revisão aprofundada com {nome_colab} "
            "na próxima 1:1, conectando esses desvios ao perfil comportamental natural para direcionar "
            "o desenvolvimento dos próximos 30 dias."
        ),
        "eixo1_roteiro": (
            f"Os desvios técnicos identificados em campo ({campo_citados}) indicam a necessidade de "
            "reforçar o cumprimento dos entregáveis formais da função durante a operação."
        ),
        "eixo1_o_que_fazer": (
            "Revisar com o líder direto, antes do próximo evento, os procedimentos padrão da função e "
            "aplicar uma checagem própria de qualidade antes de cada etapa ser dada como concluída."
        ),
        "eixo1_como_medir": "Ausência de reincidência nos mesmos itens nos próximos 2 eventos avaliados.",
        "eixo2_roteiro": (
            f"Os pilares de desempenho e governança com nota baixa ({parte_a_citados}) apontam para uma "
            "maturidade operacional ainda abaixo do esperado para o nível atual."
        ),
        "eixo2_o_que_fazer": (
            "Assumir com mais autonomia as rotinas já conhecidas da função, reportando ao gestor apenas "
            "exceções e riscos reais, não dúvidas operacionais básicas já esclarecidas anteriormente."
        ),
        "eixo2_como_medir": "Redução perceptível na necessidade de intervenção direta da liderança no próximo ciclo.",
        "eixo3_roteiro": (
            f"Os valores culturais com nota baixa ({parte_b_citados}) representam um desvio em relação aos "
            "princípios inegociáveis da Firma, e precisam de atenção imediata."
        ),
        "eixo3_o_que_fazer": (
            "Adotar uma postura de transparência total com a liderança e a equipe, comunicando problemas, "
            "erros ou imprevistos assim que identificados, sem omissões."
        ),
        "eixo3_como_medir": "Zero registros de omissão ou desvio de conduta no próximo período avaliado.",
        "fechamento_1a1": (
            "Este plano deve ser revisado formalmente entre o colaborador e o Head de Área na próxima "
            "janela de 1:1, desdobrando os três eixos em metas semanais de acompanhamento. O retorno à "
            f"elegibilidade financeira depende da consistência da virada dessas métricas para nota igual "
            f"ou superior a {NOTA_MINIMA_ELEGIVEL:.1f}."
        ),
    }
...
def gerar_plano_desenvolvimento_mensal(periodo_sel: str, ficha_mensal, compilado: dict, perfil, nome_colab: str = "", cargo_colab: str = "") -> dict:
    """Gera o PDI consolidado do mês no formato de BP de RH sênior: um
    diagnóstico integrado (causa e efeito) + 3 Macro-Eixos de Carreira
    fixos (Eficiência Técnica, Governança/Autonomia, Cultura/Valores) +
    um fechamento para a 1:1. Sempre devolve as 11 chaves obrigatórias,
    caindo no fallback offline se a chave não estiver configurada, se o
    histórico do colaborador ainda estiver incompleto ou se a API falhar."""
    if not nome_colab:
    ...
    media_parte_a = ficha_mensal["media_parte_a"] if ficha_mensal else 0.0
    media_parte_b = ficha_mensal["media_parte_b"] if ficha_mensal else 0.0
    comentarios_acumulados_campo = "; ".join(compilado["comentarios_campo"]) or "Nenhuma justificativa registrada."
    comentarios_mensais_baixos = "; ".join(compilado["comentarios_mensal"]) or "Nenhuma justificativa registrada."

    prompt_rh_senior = (
        "Você é uma Business Partner (BP) de RH Sênior e Psicóloga Organizacional na Firma Produções. "
        "Sua missão é redigir um Plano de Desenvolvimento Individual (PDI) de alto padrão, profundo e analítico para um colaborador, "
        "cruzando de forma cirúrgica seus desvios técnicos, sua avaliação de competência/cultura do mês e seu perfil comportamental natural.\n\n"
        "[DADOS CONSOLIDADOS DO COLABORADOR E DO PERÍODO]\n"
        f"- Colaborador: {nome_colab}\n"
        f"- Cargo / Nível: {cargo_colab}\n"
        f"- Período de Apuração: {periodo_sel}\n"
        f"- Média de Entrega Técnica (Campo): {compilado['media_campo_mes']:.2f}/5.0\n"
        f"- KPIs Técnicos com desvio (Notas 1 ou 2): {compilado['kpis_baixos_campo']}\n"
        f"- Justificativas/Fatos em Campo: {comentarios_acumulados_campo}\n"
        f"- Média de Desempenho e Governança (Parte A): {media_parte_a:.2f}/5.0\n"
        f"- Pilares com desvio na Ficha Mensal: {compilado['kpis_baixos_parte_a']}\n"
        f"- Média de Cultura e Valores (Parte B): {media_parte_b:.2f}/5.0\n"
        f"- Valores com desvio na Cultura: {compilado['kpis_baixos_parte_b']}\n"
        f"- Justificativas do Head na Ficha: {comentarios_mensais_baixos}\n"
        f"- Perfil Comportamental (Domínio Principal): {perfil['dominio_principal'] if perfil else 'Não preenchido'}\n"
        f"- Suas Maiores Forças (Talentos): {', '.join(t['talento'] for t in perfil['top_talentos']) if perfil else '—'}\n\n"
        "[INSTRUÇÕES RIGOROSAS DE REDAÇÃO E ARQUITETURA DO TEXTO]\n"
        "1. Escreva em segunda pessoa ('Você') utilizando uma linguagem imponente, executiva, corporativa e humanizada. Evite listas robotizadas ou tópicos repetitivos.\n"
        "2. No diagnóstico integrado, faça uma análise de causa e efeito conectando o perfil natural (forças) com os erros cometidos. Explique como a atitude e o comportamento do mês impactaram as entregas.\n"
        "3. Agrupe o plano de ação obrigatoriamente em 3 Macro-Eixos de Carreira fixos: EIXO I — EFICIÊNCIA TÉCNICA E LOGÍSTICA DE CAMPO, EIXO II — GOVERNANÇA OPERACIONAL E AUTONOMIA e EIXO III — ENGAJAMENTO CULTURAL E VALORES.\n"
        "4. Para cada Eixo, escreva um Roteiro de Alinhamento amplo explicando o desvio focado nos entregáveis formais da função, uma instrução de O que fazer (focada em atitude, inteligência emocional e rotina de trabalho) e um critério de Como medir em 30 dias baseado em entregas tangíveis e consistentes (evite menções a procedimentos de micro-engenharia ou hardware técnico específico).\n"
        "5. Conclua com um parágrafo de direcionamento para a reunião de 1:1 entre o líder e o liderado.\n"
        "Retorne APENAS um objeto JSON, sem nenhum texto antes ou depois, sem markdown, com texto fluido, denso e analítico, contendo exatamente estas chaves:\n"
        '{"diagnostico_integrado": "...", '
        '"eixo1_roteiro": "...", "eixo1_o_que_fazer": "...", "eixo1_como_medir": "...", '
        '"eixo2_roteiro": "...", "eixo2_o_que_fazer": "...", "eixo2_como_medir": "...", '
        '"eixo3_roteiro": "...", "eixo3_o_que_fazer": "...", "eixo3_como_medir": "...", '
    ...
    try:
        ...
        if all(k in parsed for k in PLANO_MENSAL_CHAVES_OBRIGATORIAS):
...
def gerar_pdf_pdi(
    ...
    # ---- Seção 1: Diagnóstico integrado (BP de RH sênior) ---------------
    story.append(Paragraph("1. ANÁLISE COMPORTAMENTAL E DIAGNÓSTICO INTEGRADO DO RH", heading_style))
    story.append(Paragraph(plano["diagnostico_integrado"], body_style))
    story.append(Spacer(1, 6))

    # ---- Seção 2: Diretrizes de evolução em 3 Macro-Eixos ---------------
    story.append(Paragraph("2. DIRETRIZES DE EVOLUÇÃO E ROTEIROS DE FEEDBACK (PRÓXIMOS 30 DIAS)", heading_style))
    for i in (1, 2, 3):
        story.append(Paragraph(EIXOS_PDI_TITULOS[i], bold_style))
        story.append(Paragraph(f"<i>Roteiro de Alinhamento:</i> {plano[f'eixo{i}_roteiro']}", body_style))
        story.append(Paragraph(f"<i>Plano de Ação:</i> {plano[f'eixo{i}_o_que_fazer']}", body_style))
        story.append(Paragraph(f"<i>Indicador de Sucesso:</i> {plano[f'eixo{i}_como_medir']}", body_style))
        story.append(Spacer(1, 8))

    # ---- Seção 3: Compromisso -----------------------------------------
    story.append(Paragraph("3. ROTEIRO DE COMPROMISSO E DIRETRIZES PARA A REUNIÃO DE 1:1", heading_style))
    story.append(Paragraph(plano["fechamento_1a1"], body_style))
    story.append(Spacer(1, 6))
