...
- **Tracking dos títulos corrigido.** O manual pede tracking **negativo**
  (-3%) em títulos, caixa alta, peso Black/Bold — o CSS anterior usava
  espaçamento positivo (errado). Corrigido para `-0.03em` + peso 800.
- **Slogan oficial** ("Para toda boa ideia, uma boa solução.") adicionado
  na tela de login, abaixo do logotipo — o mesmo texto que já aparece no
  papel timbrado.
- **Grafismo das setas** (o elemento gráfico oficial da marca, derivado do
  "play" do descritor "PROD.") aplicado como uma faixa horizontal na tela
  de login, seguindo a orientação do próprio manual ("recorte uma tira
  horizontal para separar seções... a ponta das setas continua
  aparecendo"). Está embutido em base64 na constante `FAIXA_SETAS_B64`.
  Por indicação explícita do manual ("na dúvida, menos é mais: um bloco de
  setas por peça"), não repeti esse grafismo em outras telas.
- **Cores conferidas e batendo 100%** com a paleta oficial: Preto Firma
  `#111111`, Vermelho Firma `#E12E2E`, Branco `#FFFFFF`.

**Não apliquei** (por não termos os arquivos ou não caber no contexto de
um app interno):
- A cor **Vermelho Escuro** `#B81F1F` (recomendada para hover e textos
  pequenos em vermelho sobre fundo branco) — nosso app é majoritariamente
  fundo escuro, então essa regra específica de contraste não se aplica do
  mesmo jeito; o tema nativo do Streamlit já ajusta o hover dos botões
  automaticamente a partir do `primaryColor`.
- O **Cinza Grafite** `#4A4A4A` para textos secundários — testei e esse
  tom específico fica com contraste baixo demais sobre o `#111111` do
  fundo escuro do app (ele foi pensado para textos secundários sobre fundo
  claro, como documentos e papelaria). O tema nativo já usa um tom de
  texto secundário com contraste adequado para o fundo escuro.
